...
def generate_continuous_ember():
    return Boid(
        WIDTH // 2 + random.uniform(-30, 30),  # Start near center
        HEIGHT - 50,  # Near bottom
        random.uniform(-1, 1),  # Random horizontal velocity
        random.uniform(-3, -1),  # Random upward velocity
        random.uniform(100, 200)  # Random lifetime
    )

# Initialize continuous embers
NUM_CONTINUOUS_EMBERS = 100
embers = [generate_continuous_ember() for _ in range(NUM_CONTINUOUS_EMBERS)]
...
while running:
    ...
    for ember in embers[:]:
        ...
        alpha = max(0, int((ember.lifetime / 250) * OPACITY))  # Adjust denominator as needed
        ...
        if ember.lifetime <= 0 or ember.y < 0:
            ...
            embers.append(generate_continuous_ember())

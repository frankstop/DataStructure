...
# Define the AVLTree class with rotation tracking
class AVLTree:
    def __init__(self):
    ...
    # Clear rotation info after handling
    def clear_rotations(self):
        self.rotation_info = []

# Define the AVLTreeVisualizer class with enhanced visualization
class AVLTreeVisualizer:
    ...
    # Plot the AVL tree with enhanced visualization
    def plot_tree(self):
        self.ax.clear()
        if self.root is not None:
            ...
            for node_key, (x, y) in self.positions.items():
                ...
                # Highlight imbalance nodes
                if node_key in self.imbalance_nodes:
                    color = 'salmon'

                self.ax.scatter(x, y, s=1000, c=color, edgecolors=edgecolor, zorder=3)
    ...
    def submit_insert(self, text):
        ...
        try:
            key = int(text)
            # Clear previous highlights and annotations
    ...
    # Handle rotation events for visualization
    def handle_rotations(self):
        if self.tree.rotation_info:
            # Take the last rotation event
            last_event = self.tree.rotation_info[-1]
            rotation_type = last_event['type']
            node = last_event['node']
            rotated_node = last_event['rotated_node']

            # Find positions to annotate
            node_pos = self.positions.get(node, (0,0))
            rotated_node_pos = self.positions.get(rotated_node, (0,0))

            # Highlight the nodes involved in rotation
            self.imbalance_nodes = [node, rotated_node]
            ...
            print(f"Performed {rotation_type} on node {node}")

        # Clear rotation info after handling
        self.tree.clear_rotations()
...
if __name__ == "__main__":
    ...
    for key in initial_insertions:
        ...
        print()
    # After all initial insertions, handle any rotations
    visualizer.handle_rotations()
